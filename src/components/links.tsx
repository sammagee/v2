...
export default function Links() {
  ...
  return (
    <div className="fixed z-10 flex-col items-center hidden space-y-3 bottom-4 md:flex md:left-8 md:right-auto right-8 h-2/5">
      <Tooltip
        content={
      ...
      <div className="flex-1 w-[2px] opacity-40 dark:opacity-25 border border-dashed border-brand-400 rounded-t-full" />

      <button
        className={clsx(
          'text-brand-400 opacity-40 dark:opacity-25 transform transition ease-in-out duration-200 p-2 !mt-0 focus:outline-none rounded-full border-2 border-dashed border-transparent focus:border-brand-500',
          scrollPos <= 250 && 'cursor-default',
          scrollPos > 250 && 'rotate-180 hover:opacity-100 dark:hover:opacity-100 hover:bg-brand-400 hover:bg-opacity-10 hover:text-brand-500',
        )}
